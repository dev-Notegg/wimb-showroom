...
const App = () => {
  ...
  const handleConfirmCrop = () => {
    ...
    const cropBoxX = cropTopLeft.x;
    const cropBoxY = cropTopLeft.y;

    // crop 박스 기준점에서 이미지 위치까지 벡터
    const dx = cropBoxX - imgX;
    ...
    const scaleXFactor = (img.crop?.width || 0) / imageWidth;
    const scaleYFactor = (img.crop?.height || 0) / imageHeight;

    const cropX = (img.crop?.x || 0) + localX * scaleXFactor;
    const cropY = (img.crop?.y || 0) + localY * scaleYFactor;
